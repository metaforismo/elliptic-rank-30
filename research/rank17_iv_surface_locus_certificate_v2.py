#!/usr/bin/env python3
"""Build the canonical F11 IV surface-locus certificate from the C scan.

The accepted tuples are reconstructed with the corrected independent Python
implementation.  At F5 the entire C accepted set is compared with a separate
full Python enumeration.
"""
...
IMPLEMENTATION = HERE / "rank17_iv_surface_locus_finite_field_v2.py"


def load_implementation():
    spec = importlib.util.spec_from_file_location(
        "rank17_iv_surface_locus_finite_field_v2", IMPLEMENTATION
    )
...
def rebuild_records(raw: dict[str, object], implementation) -> list[dict[str, object]]:
    ...
    for values in raw["tuples"]:
        ...
        if implementation.tuple_key(record) != values:
...
def cross_check_small_prime(
    ...
    python_tuples = [
        implementation.tuple_key(record)
        for record in exhaustive["candidates"]
    ]
    c_tuples = list(raw_small["tuples"])
    if c_tuples != python_tuples:
        raise AssertionError({
            "prime": prime,
            "missing_from_c": sorted(set(python_tuples) - set(c_tuples))[:20],
            "extra_in_c": sorted(set(c_tuples) - set(python_tuples))[:20],
        })
    return {
        "prime": prime,
...
def build_certificate(raw_production: Path, raw_small: Path) -> dict[str, object]:
    ...
    payload: dict[str, object] = {
        ...
        "implementation": {
            ...
            "independent_verifier": "research/rank17_iv_surface_locus_finite_field_v2.py",
            "certificate_builder": "research/rank17_iv_surface_locus_certificate_v2.py",
        },
    }
