#!/usr/bin/env python3
"""Exhaustively enumerate the normalized I12 + I4 + IV surface locus over F_p.

This script isolates the surface equations before any section variables are
introduced.  The normalized short Weierstrass model is

    y^2 = x^3 - 3*c4(t)*x - 2*c6(t),

with c4 monic of degree 8 and c6 monic of degree 12.  The constraints are:

* I12 at infinity, imposed by the truncated formal square root
  c6^vee = (c4^vee)^(3/2) mod w^12;
* I4 at t=0, imposed by square/cube jets through orders 3;
* IV at t=1, imposed by c4(1)=c4'(1)=c6(1)=c6'(1)=0 and
  the exact non-vanishing of the quadratic c6 coefficient;
* exact I4 and I12 orders;
* split tangent cones at I4 and IV.

Every tuple in the output is checked directly from the full discriminant.
The computation is entirely over a finite field and makes no Q-rational or
rank-30 claim.
"""
...
def c4_from_parameters(
    ...
    # Write S0=L0+r and S1=L1+6r.  Solving
    # a4+a5=-S0 and 4*a4+5*a5=-S1 gives the formulas below.
...
def surface_record(
    *,
    ...
    c6 = c6_from_c4_and_constant(e0 * pow(p0, 3, prime), prime)
    target_jets = [
        3 * e0 * p0 * p0 * p1,
...
def enumerate_surfaces(prime: int) -> dict[str, object]:
    ...
    candidates.sort(key=lambda item: tuple(
    ...
    payload: dict[str, object] = {
        ...
        "truth_status": (
            f"EXHAUSTIVE normalized I12+I4+IV surface enumeration over F_{prime}; "
        ...
        "limitations": [
            "Only the declared normalized surface chart is enumerated.",
            "The section incidence equations are not imposed by this certificate.",
            "Finite-field points need not lift to Q or to any characteristic-zero field.",
        ],
    }
