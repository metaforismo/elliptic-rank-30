...
## Determinant and its primitive part

Eliminating \(z\) from (4)--(5) yields the determinant quintic
...
Its coefficient content is exactly \(11\). Therefore write

\[
P(t)=11P_0(t),
\]

where the primitive part is

\[
\begin{aligned}
P_0(t)={}&14064t^5-68944t^4+24221153t^3\\
         &-48533826t^2+32485596t-7251288.
\end{aligned} \tag{7}
\]

The executable certificate verifies (6)--(7) in three independent exact
ways:

1. directly in a sparse Laurent-polynomial ring over \(\mathbf Q\);
2. by integer coefficient convolution of the two \(2\times2\) determinant
   products;
3. by computing the coefficient gcd and checking \(P=11P_0\) coefficient by
   coefficient.

The primitive polynomial \(P_0\) has
...
\[
5,13,18,1,7,17,10,6,14,18,1,7,13,10,8,3,1,4,15.
\]

None is zero.

If a primitive integer polynomial has a rational root \(m/n\) in lowest terms,
then \(n\) divides its leading coefficient. Since \(19\) does not divide
\(14064\), the denominator is invertible modulo \(19\), and \(m/n\) would
reduce to a root of \(P_0\) in \(\mathbf F_{19}\). The exhaustive value table
proves that this is impossible. Since \(P\) and \(P_0\) have the same rational
roots, the determinant cannot vanish at a rational \(t\).
...
```bash
PYTHONPATH=. python3 research/degree47_target_mod19_obstruction.py \
  --check certificates/degree47_target_mod19_obstruction.json
PYTHONPATH=. python3 -m unittest \
  tests.test_degree47_target_mod19_obstruction -v
```
